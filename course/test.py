...
def get_words_with_ignore(doc, zabraneti_zborovi=None):
    words = set()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20:
            if zabraneti_zborovi is not None and word.lower() not in words_to_ignore:
                words.add(word.lower())
    return words
...
def proba():
    ...
    res = sorted(list(recnik.items()), key=lambda x: x[1], reverse=True)[0][0]

    # recnik = classify(nesto, nekoe_drvo)
    klasa = najdi_kalas(recnik)
