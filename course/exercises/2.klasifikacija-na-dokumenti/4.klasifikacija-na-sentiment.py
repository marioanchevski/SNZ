""" Класификација на сентимент Problem 4 (2 / 2)
...
def get_emoticons(doc, emoticons=None):
    words = set()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20:
            words.add(word.lower())
...
def helper_func(doc):
    return get_emoticons(doc,emoticons)
...
if __name__ == '__main__':
    ...
    klasa2 = NaiveBayes(helper_func)
    test_data=data.pop(sample_ind)

    for d in data:
        klasa1.train(d[1],d[0])
        klasa2.train(d[1],d[0])

    x1= klasa1.classify_document(test_data[1])
    x2 = klasa2.classify_document(test_data[1])

    print(test_data[1])
    print("Vistinska klasa: " +str(test_data[0]))
    print("Klasa predvidena so Naiven Baes (bez emotikoni): " +str(x1))
    print("Klasa predvidena so Naiven Baes (so emotikoni): " +str(x2))