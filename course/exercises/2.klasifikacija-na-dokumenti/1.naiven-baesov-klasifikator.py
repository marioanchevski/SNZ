"""Наивен баесов класификатор Problem 3 (3 / 4)
...
if __name__ == '__main__':
    sample = input()
    training_data = train_data[: int(len(train_data)*0.7)]
    test_data = train_data[int(len(train_data)*0.7):]

    klasa = NaiveBayes(get_words)
    for d in training_data:
        klasa.train(d[0],d[1])
    brojac=0
    for t in test_data:
        x = klasa.classify_document(t[0])
        if x == t[1]:
            brojac+=1

    print(brojac/len(test_data))
    print(klasa.classify_document(sample))