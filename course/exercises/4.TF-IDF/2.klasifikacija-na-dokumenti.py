"""
Изградете два модели за класификација на документи, со кои ќе се одредува дали даден документ е формален или неформален.
Првиот модел е модел на Наивен баесов класификатор. Вториот класификатор е дрво на одлука каде што документите се претставени со tf-idf вектори.
Тренирајте ги двата класификатори со соодветното множество за тренирање.

За документот прочитан од стандарден влез испечатете ја класата во која тој припаѓа.
Доколку со двата поединечни класификатори се добие истата класа, тогаш се печати таа. Во спротивно се печати непозната (unknown) класа.
"""
...
if __name__ == '__main__':
    ...
    baesov = NaiveBayes(get_words)
    for item in train_data:
        baesov.train(item[0], item[1])

    baesov_prediction = baesov.classify_document(sample)

    docs, labels = get_dataset(train_data)
    dataset, df, N, vocab = create_dataset(docs, labels)
    drvo = build_tree(dataset)

    sample_transformed = process_document(sample, df, N, vocab)
    drvo_prediction = max(classify(sample_transformed, drvo), key=operator.itemgetter(1))

    if baesov_prediction == drvo_prediction:
        print(baesov_prediction)
    else:
        print("unknown")
