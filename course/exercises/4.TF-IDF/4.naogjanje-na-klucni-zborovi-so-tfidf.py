...
Најпрво, потребно е речениците да се претстават како tf-idf зборовни вектори.
Од стандарден влез се чита листа на индекси на документи (sentences) за кои треба да се извлечат клучните зборови.
За клучни зборови се сметаат оние зборови со најголема tf-idf вредност, која задоволува одреден праг на одлучување (threshold).
Овој праг е прочитан од стандарден влез во променливата threshold.
За секој од бараните реченици испринтајте ги 5те клучни зборови на стандарден излез (доколку има помалку од 5 клучни зборови испринтајте ги,
а доколку нема клучни зборови, тогаш испринтајте "No keywords ...").
"""
import math
import re
import operator
...
if __name__ == '__main__':
    ...
    for index in sentences:
        vector = process_document(docs[index], df, n, vocab)
        keywords = []

        for element in vector:
            if element[0] >= threshold:
                keywords.append(element)
        keywords.sort(key=operator.itemgetter(0), reverse=True)
        keywords = keywords[:5]
        if len(keywords) != 0:
            string = f'{index} ->'
            ...
            string = f'{index} -> No keywords ...'
            print(string)

        # if keywords != []:
        #     keywords = sorted(keywords, key=operator.itemgetter(0), reverse=True)[:5]
        #     key2 = []
        #     for item in keywords:
        #         new = str(item[0])
        #         key2.append((item[1], new))
        # res = [': '.join(tups) for tups in key2]
        #
        # print(f'{i} -> ' + ', '.join(res))
