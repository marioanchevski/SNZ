"""
Дадено е податочно множество (dataset) на спам коментари. Потребно е да се направи класификатор на коментари според tf-idf векторските репрезентации на коментарите.
Овој класификатор ја предвидува класата на моменталниот коментар според класата која е мнозинство од класите на N-те најслични коментари претставени со tf-idf вектори.

Од стандарден влез се чита бројот на слични коментари N во променливата n,
како и коментар кој треба да се класифицира во spam или ham класа. За да се предвиди класата, најпрво потребно е да се најдат n најслични коментари според косинусно растојание,
по што класата треба да се определи според доминантната класа од множеството на класи добиени од најсличните коментари.
Ако не се пронајдени N најслични документи со сличност поголема од 0,
потребно е да се разгледуваат само добиените најслични документи (чиј што број е помал од N).
 Доколку бројот на двете класи е еднаков треба да се предвиди класата spam.
"""
...
if __name__ == '__main__':
    ...
    slicni = rank_documents(comment, docs, cosine)[:n]
    spam_votes = 0
    ham_votes = 0
    for item in slicni:
        vote = dataset[item[1]][1]
        if vote == 'spam':
            spam_votes += 1
        else:
            ham_votes += 1
    ...
    print(f'Predvidenata klasa e {klasa} so {max(spam_votes,ham_votes)}/{n} glasovi.')