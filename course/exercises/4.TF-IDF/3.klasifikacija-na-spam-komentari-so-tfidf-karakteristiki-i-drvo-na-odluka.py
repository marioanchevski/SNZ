...
dataset = [('CONTENT', 'ham'), ('+447935454150 lovely girl talk to me xxx', 'spam'),
           ...
           ('857.482.940 views AWESOME !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!', 'ham')] #(document, class)
...
if __name__ == '__main__':
    ...
    data, df, n, vocab = create_dataset(docs, labels)
    tree = build_tree(data)

    TP, FP, TN, FN = 0, 0, 0, 0
    for red in test_data:
        test_vec = process_document(red[0], df, n, vocab)
        c = max(classify(test_vec, tree).items(), key=lambda x: x[1])[0]
        if red[1] == 'spam':
            if c == red[1]:
                TP += 1
            else:
    ...
    test_comment_trans = process_document(test_comment, df, n, vocab)
    drvo_predict = max(classify(test_comment_trans, tree).items(), key=operator.itemgetter(1))[0]
