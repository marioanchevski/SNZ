"""
Дадено е податочно множество на документи од спорт (спорт) и наука (science). Потребно е да се најдат најсличните 4 документи на документ кој се чита од стандарден влез (test_doc).
Сличноста на документите се одредува според клучни зборови пронајдени со методот tf-idf,
односно сличноста се однесува на бројот на заеднички клучни зборови. Колку е поголем бројот на заеднички клучни зборови,
толку тој документ е посличен со моменталниот документ.

Секој од документите треба да се престави со tf-idf вектор,
од кој треба да се извлечат зборовите со tf-idf вредност поголема од 0 како клучни зборови.
За документот кој се чита од стандарден влез потребно е да се извлечат клучните зборови и да се спореди со документите од податочното множество (да се најде бројот на заеднички клучни зборови).
На стандарден излез испечатете ги индексите на 4те најслични документи од множеството за тренирање,
...
if __name__ == '__main__':
    ...
    test_doc_vector = process_document(test_doc, df, len(docs), vocab)
    test_doc_keywords = []
    for item in test_doc_vector:
        if item[0]>0:
            test_doc_keywords.append(item[1])
    test_doc_keywords = set(test_doc_keywords)
    ...
    for item in docs:
        counter = 0
        vector = process_document(item, df, len(docs), vocab)
        keywords = []
        zborovi = []
        for item in vector:
            if item[0]>0:
        ...
        for word in test_doc_keywords:
            for word2 in keywords:
                if word == word2:
                    counter+=1
                    zborovi.append(word)

        slicni.append((document_no,sorted(zborovi),counter)) #(br_dokument,zborovi koi se isti, broj na isti klucni zborovi)
        document_no += 1

    final=sorted(slicni, key=operator.itemgetter(2), reverse=True)[:4]
    for item in final:
        print(f'Dokument #{item[0]}: '+' '.join(item[1]))