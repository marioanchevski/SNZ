...
if __name__ == "__main__":
    ...
    roach_set = [row for row in data if row[-1]=='Roach']
    pike_set = [row for row in data if row[-1]=='Pike']

    train_set = roach_set[:40] + pike_set[:40]
    tree = build_tree(train_set)

    predicted_class = None
    predicted_class = classify(test_case,tree)

    print(max(predicted_class.items(), key=operator.itemgetter(1))[0])
