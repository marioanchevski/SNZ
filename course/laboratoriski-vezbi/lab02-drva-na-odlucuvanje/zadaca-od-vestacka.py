...
def build_tree(rows, scoref=entropy,level=0):
    ...
    if best_gain > 0:
        true_branch = build_tree(best_sets[0], scoref,level+1)
        false_branch = build_tree(best_sets[1], scoref,level+1)
        return DecisionNode(col=best_criteria[0], value=best_criteria[1],
                            tb=true_branch, fb=false_branch,level=level)
    else:
        return DecisionNode(results=unique_counts(rows),level=0)
...
def classify(observation, tree):
    ...
    if tree.results:
        ...
        if compare(observation, tree.col, tree.value):
            branch = tree.tb
            branch_str= "True branch"
        else:
            branch = tree.fb
            branch_str = "False branch"

        string = "(" + str(tree.col) + ", '" + str(tree.value) + "')"
        print("Sporeduvam kolona i vrednost "+ string)
        print("Tekovna vrednost: " + str(value))
        print("Sledna granka: "+ branch_str)
...
if __name__ == "__main__":
    ...
    print_tree(tree)
    predicted_class = classify(testCase, tree)
    klasa = max(predicted_class.items(), key=operator.itemgetter(1))[0]
    print("\n"+klasa)
