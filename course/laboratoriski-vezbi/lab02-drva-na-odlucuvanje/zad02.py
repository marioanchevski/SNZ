...
if __name__ == "__main__":
    ...
    bream_set = [row for row in data if row[-1] == "Bream"]
    perch_set = [row for row in data if row[-1] == "Perch"]

    train_set = bream_set[:25] + perch_set[:25]
    tree = build_tree(train_set)

    predicted_class = classify(test_case,tree)
    print(predicted_class)
    klasa = max(predicted_class.items(), key=operator.itemgetter(0))[0]
    print(klasa)