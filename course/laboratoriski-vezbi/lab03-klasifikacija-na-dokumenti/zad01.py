...
def istreniraj(cl):
    for row in train_data:
        dokument = row[0]
        klasa = row[1]
        cl.train(dokument, klasa)

if __name__ == "__main__":

    klasifikator = NaiveBayes(get_words)
    istreniraj(klasifikator)

    recenica = input()
    predvidena_klasa = klasifikator.classify_document(recenica)
    print(predvidena_klasa)

    verojatnost = klasifikator.get_category_probability_for_document(recenica, predvidena_klasa)

    print(round(verojatnost, 10))