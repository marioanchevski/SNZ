...
def get_words_new(doc, em=None):
    words = set()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20:
            words.add(word.lower())
    for word in doc.split(" "):
        if em is not None and word.lower() in emoticons:
            words.add(word.lower())
    return word
...
if __name__ == '__main__':
    ...
    for i in range(start_index, end_index):
        dodadi = data[i]
        test_data.append(dodadi)

    for row in data:
        if row not in test_data:
            train_data.append(row)



    klasifikator1  = NaiveBayes(get_words)
    klasifikator2 = NaiveBayes(lambda x: get_words_new(x, emoticons))

    treniraj(klasifikator1)
    treniraj(klasifikator2)

    for i in range(len(test_data)):
        klasa = test_data[i][0]
        dokument = test_data[i][1]
        print(f'{i + 1}:')
        print(dokument)
        print("Vistinska klasa:", klasa)
        print("Klasa predvidena so Naiven Baes (bez emotikoni):",klasifikator1.classify_document(dokument))
        print("Klasa predvidena so Naiven Baes (so emotikoni):",klasifikator2.classify_document(dokument))
