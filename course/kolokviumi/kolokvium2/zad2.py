"""
Дадено е податочно множество на коментари на Youtube видеа од музичкиот изведувач Еминем.
Секој од коментарите припаѓа на една од класите "ham" или "spam".
Поделете го податочното множество на множество за тренирање, во кои ќе се вклучат првите 75% од податочното множество,
 и множество за тестирање со останатите инстанци од податочното множество.

Изградете модел на дрво на одлука за класификација на коментарите со тренирачкото множество,
каде што карактеристиките за секој коментар ќе бидат добиени од зборовниот вектор креиран со td-idf.
Пресметајте точност, одзив и прецизност на моделот со тестирачкото множество и испринтајте ги вредностите на стандарден излез.
...
if __name__ == '__main__':
    ...
    dataset_drvo, df, N, vocab = create_dataset(docs, labels)

    tree = build_tree(dataset_drvo)

    tp, tn, fp, fn = 0, 0, 0, 0
    for item in test_set:
        doc = process_document(item[0], df, N, vocab)
        drvo_prediction = max(classify(doc, tree).items(), key=operator.itemgetter(1))[0]
        if item[-1] == 'spam':
            if drvo_prediction == 'spam':
                tp+=1
            else:
                fn+=1
        else:
    ...
    test_comment_processed = process_document(test_comment, df, N, vocab)
    drvo_prediction = max(classify(test_comment_processed, tree).items(), key=operator.itemgetter(1))[0]
    print(f'Tochnost: {tochnost}')
    print(f'Preciznost: {preciz}')
    print(f'Odziv: {odziv}')
    print(drvo_prediction)